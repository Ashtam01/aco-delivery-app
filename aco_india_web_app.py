...
if uploaded_file:
    ...
    if not required_cols.issubset(df.columns):
        ...
        # City selection
        n = st.number_input("Enter number of delivery points (Min: 3)", min_value=3, max_value=len(cities), value=4)
        location_names = list(cities.keys())
        start_city = st.selectbox("Select Start Location", location_names)
        end_city = st.selectbox("Select End Location", [c for c in location_names if c != start_city])
        remaining = [c for c in location_names if c not in [start_city, end_city]]
        num_intermediate = n - 2
        intermediate_cities = st.multiselect(f"Select {num_intermediate} Intermediate Locations", options=remaining, max_selections=num_intermediate)
        ...
        if len(intermediate_cities) != num_intermediate:
            ...
            def calculate_distance(c1, c2):
                return geodesic(cities[c1], cities[c2]).km
            ...
            st.subheader("🗺️ Route Map (Red = Unoptimized, Blue = Optimized)")
